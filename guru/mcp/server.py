...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "If network.reachable is false: run unlock once, show "
        "rider_facing.text once (user-visible UX copy, not a system prompt), "
        "then wait for a new chat. Still blocked / no shell: one "
        "human_fix recipe. Never invent steps; never ask for pipx. Never "
        "unlock when already reachable. wire_mcp ONLY if shell guru "
        "unavailable. Upgrade if update_available. get_profile; if "
